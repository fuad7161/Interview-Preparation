...
void dfs(int x,int y,int dif){
    ...
    vis[x][y] = true;
    int up = x-1;
    while(up >= 0 and a[up][y] == 0){
        up--;
    }
    if(up >= 0 and vis[up][y] == false){
        dfs(up , y , max(dif , x - up));
    }
    int down = x+1;
    while(down<n and a[down][y] == 0){
        down++;
    }

    if(down < n and vis[down][y] == false){
        dfs(down , y , max(dif , down - x));
    }

    if(y+1 < m and vis[x][y+1] == false and a[x][y+1]!=0){
        dfs(x,y+1,dif);
    }

    if(y-1>=0 and vis[x][y-1] == false and a[x][y-1]!=0){
        dfs(x,y-1,dif);
    }
