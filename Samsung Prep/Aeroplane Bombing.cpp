...
int n;
int ans = 0;
void fun(int level , int x,int cnt,int encounter,int valCnt){
    if(level == 0)return;
    // error(encounter, valCnt)
    ans = max(ans , cnt);
    int tlevel = level-1;
    for(int i=-1;i<=1;i++){
        int ux = x+i;
        if(ux >= 0 and ux <5){
            bool tencounter = (encounter|(a[tlevel][ux]==2));
            int tvalcnt = valCnt+tencounter;
            int tcnt = cnt;
            if(a[tlevel][ux] == 1){
                tcnt++;
            }
            if(a[tlevel][ux] == 2 and valCnt >=5)continue;
            fun(tlevel , ux,tcnt , tencounter , tvalcnt);
        }
    }
}
