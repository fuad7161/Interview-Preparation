...
int32_t solve() 
{
    ...
    vector<wormhole>v(n);
    for(int i=0;i<n;i++){
        cin>>v[i].px>>v[i].py>>v[i].fx>>v[i].fy>>v[i].cost;
        v[i].cost = min(v[i].cost , abs(v[i].px - v[i].fx)+abs(v[i].py - v[i].fy));
    }

    auto built = [&](vector<wormhole>t)->int{
        int sum = 0;
        int px1 = x1;
        int py1 = y1;
        for(int i=0;i<t.size();i++){
            sum += abs(px1 - t[i].px)+abs(py1 - t[i].py);
            sum += t[i].cost;
            px1 = t[i].fx;
            py1 = t[i].fy;
        }
        sum += (abs(px1-x2)+abs(py1-y2));
        return sum;
    };

    auto make = [&](vector<wormhole>&t)->int{
        vector<int> a;
        // for(auto i: t){
        //     error(i.px,i.fx,i.py,i.fy,i.cost)
        // }cout<<endl;
        for(int i=0;i<t.size();i++){
            a.push_back(i);
        }
